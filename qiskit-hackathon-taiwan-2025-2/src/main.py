...
if __name__ == '__main__':
    ...
    gate_types = config['MOL'].get('gate_types', fallback='h,x,y,z,cx,cz,rx,ry,rz,t,s,sx')
    ...
    for i in range(num_episodes):
        ...
        while not (done or truncated) and step_count < num_steps:
            ...
            if 'ep_energy' in info:
                episode_energy = info['ep_energy'][-1]
        
        # Learn from experience after each episode
        agent.learn()
        
        # Save best circuit
        if abs(episode_energy - fci_energy) < abs(best_energy - fci_energy):
        ...
        energy_difference.append(np.log10(abs(episode_energy-fci_energy)))
        ...
        if (i+1) % 100 == 0:
            ...
            plt.subplot(1,3,2)
            plt.plot(energy_difference)
            plt.axhline(y=[0]*len(fci_energy), color='r', linestyle='--')
            plt.xlabel('episode')
            plt.ylabel('energy difference (Ha)')
            plt.title('Energy Difference (Log) Plot')
            ...
            plt.xlabel('episode')
            plt.ylabel('reward')
            plt.title('Reward Convergence Plot')
            plt.savefig(f"results/episode_{i+1}_converge.png")
            
            # Save circuit diagram
            plt.subplot(133)
            best_circuit.draw(output='mpl')
            plt.savefig(f"results/episode_{i+1}.png")
